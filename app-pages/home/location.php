<?php
/**
* @package :    LAZARUS
* @since   :    V 1.0
* name     :    location template  
*/
?>

<section id="locations" class="section section--padding__wmargin">
    <div class="container">
        <div class="row">
            <div class="preamble col-lg-8 offset-lg-2">
                <h3 class="title">Lazarus Network Nodes Locations</h3>
                <p class="lead">We cover entire globe. Now you can access our VPN from anywhere your want.</p>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <img src="https://lazarus.network/wp-content/themes/lazarus/app-resources/images/map.png" alt=""
                    class="map img-fluid">
                <div class="location location-germany" data-toggle="tooltip" data-html="true"
                    title="<h5>United Kingdom</h5><p>2 Nodes</p>">
                    <div class="location__pin"></div>
                    <div class="location__rwd d-block d-md-none">
                        <h5>Germany</h5>
                        <p>2 Nodes</p>
                    </div>
                </div>
                <div class="location location-canada" data-toggle="tooltip" data-html="true"
                    title="<h5>Canada Node</h5><p>2 Nodes</p>">
                    <div class="location__pin"></div>
                    <div class="location__rwd d-block d-md-none">
                        <h5>Canada Node</h5>
                        <p>2 Nodes</p>
                    </div>
                </div>
                <div class="location location-usa" data-toggle="tooltip" data-html="true"
                    title="<h5>USA Node</h5><p>4 Nodes</p>">
                    <div class="location__pin"></div>
                    <div class="location__rwd d-block d-md-none">
                        <h5>USA Node</h5>
                        <p>4 Nodes</p>
                    </div>
                </div>
                <div class="location location-singapore" data-toggle="tooltip" data-html="true"
                    title="<h5>Singapore Node</h5><p>2 Nodes</p>">
                    <div class="location__pin"></div>
                    <div class="location__rwd d-block d-md-none">
                        <h5>Singapore Node</h5>
                        <p>2 Nodes</p>
                    </div>
                </div>
                <div class="location location-india" data-toggle="tooltip" data-html="true"
                    title="<h5>India Node</h5><p>2 Nodes</p>">
                    <div class="location__pin"></div>
                    <div class="location__rwd d-block d-md-none">
                        <h5>India Node</h5>
                        <p>2 Nodes</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>